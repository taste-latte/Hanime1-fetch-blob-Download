...
// @updateURL     https://github.com/taste-latte/Hanime1-fetch-blob-Download/tree/main/Hanime1 [fetch + blob] Download.meta.js
// @downloadURL   https://github.com/taste-latte/Hanime1-fetch-blob-Download/tree/main/Hanime1 [fetch + blob] Download.user.js
// @version      1.1
// @description  下載按鈕放訂閱旁，自動正確命名影片[使用(fetch + blob)強制命名]，紀錄下載的影片並且下次下載相同影片會提示已下載是否下載
// @match        *://hanime1.me/*
// @grant        none
// ==/UserScript==
...
(function () {
    ...
    async function startDownload(videoId) {
        ...
        // ✅ 判斷是否已下載過
        const downloaded = JSON.parse(localStorage.getItem('hanime1_downloaded') || '[]');
        if (downloaded.includes(finalName)) {
            const reDownload = confirm(`⚠️ 此影片已下載過：\n${finalName}\n\n是否要重新下載？`);
            if (!reDownload) {
        ...
        try {
            ...
            // ✅ 記錄下載成功的檔案名
            downloaded.push(finalName);
            localStorage.setItem('hanime1_downloaded', JSON.stringify([...new Set(downloaded)]));
